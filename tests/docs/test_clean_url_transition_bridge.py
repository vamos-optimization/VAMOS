...
from tools.check_docs_portal import PortalCheckError
from tools.check_docs_portal_transition import check_transition_portal
...
def test_transition_bridge_accepts_legacy_portal_contract(tmp_path: Path) -> None:
    ...
    assert result["contract"] == "legacy-stable-tree"
...
def test_transition_bridge_accepts_only_explicit_clean_current_contract(tmp_path: Path) -> None:
    ...
    result = check_transition_portal(root, version=VERSION, base_url=BASE_URL)
    assert result["contract"] == "clean-current-transition"

    _write(root, "docs/stable/index.html", _redirect("https://evil.invalid/"))
    with pytest.raises(PortalCheckError, match="neither approved transition contract"):
        check_transition_portal(root, version=VERSION, base_url=BASE_URL)
