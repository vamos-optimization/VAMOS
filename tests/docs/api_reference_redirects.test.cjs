...
for (const base of [
  "https://vamos-optimization.org/docs/stable/reference/api_reference/",
  "https://vamos-optimization.org/docs/1.0.0/reference/api_reference/",
  "https://pr-33-vamos-docs-preview.vamos-optimization.workers.dev/docs/stable/reference/api_reference/",
  "http://127.0.0.1:8001/reference/api_reference/",
]) {
  test(`all frozen bookmarks keep their origin and version: ${base}`, () => {
...
test("unknown, malformed, empty, and ordinary heading fragments stay on the index", () => {
  for (const hash of ["", "#missing-symbol", "#%", "#api-reference", "#core-optimization"]) {
    assert.deepEqual(run(`https://example.org/docs/stable/reference/api_reference/${hash}`).replacements, []);
  }
});
test("unrelated pages and unsafe targets cannot redirect", () => {
  const base = "https://example.org/docs/stable/reference/api_reference/#unified-api";
...
test("a hash change after initial load is handled", () => {
  const result = run("https://example.org/docs/stable/reference/api_reference/");
  result.location.hash = "#unified-api";
  result.location.href += "#unified-api";
  result.listeners.hashchange();
  assert.deepEqual(result.replacements, ["https://example.org/docs/stable/reference/api/optimization/"]);
});
